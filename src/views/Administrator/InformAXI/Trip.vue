...
export default {
  ...
  computed: {
    ...
    ...mapGetters('informaxi', ['informaxi_trip']),
  },
  methods: {
    filesChange(ev) {
    ...
    async getData() {
      ...
      let res = JSON.parse(JSON.stringify(await this.getInformaxiTrip(data)));
      if (res.name != 'Error') {
        const that = this
        await this.getAllCabangWithArea()

        this.item = JSON.parse(JSON.stringify(this.informaxi_trip))
        this.item.map((data) => {
    ...
    async uploadFile() {
      ...
      for (let i = 0; i < this.upload_info.files.length; i++) {
        ...
        let res = await this.updateInformaxiTrip(payload)
        if (res.name == "Error") {
          success = false
