    ...
    <div>
        <base-header type="">
            <div class="row align-items-center py-4">
                <div class="col-lg-12">
                    <h6 class="h2 d-inline-block mb-0">Simulation - Motor</h6>
            ...
            <div class="row align-items-center py-4">
                ...
                <div class="col-lg-6 pt-2">
                    <div class="d-flex justify-content-end">
                        ...
                        <a :href="UrlHealer(`${$store.state.simulation_base_url}/mrp/mcy/download${mrp.area ? `?area_id=${mrp.area}` : ``}`)"
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="text-uppercase">
                        {{ findInArray(mrp.area, area_lists) }} ({{ findInArray(mrp.versi_dokumen_id, list_versi_dokumen) }})
                    </h3>
                </div>
    ...
    methods: {
        async getMrp() {
            try {
                ...
                let mrp = await this.$api.get(
                    `${this.$store.state.simulation_base_url}/mrp?tipe_objek_id=2&area_id=${this.mrp.area}&versi_dokumen_id=${this.mrp.versi_dokumen_id}&offset=${this.mrp.entry}&page=${this.mrp.current_page}${this.mrp.search_type && this.mrp.search_query ? `&search=${this.mrp.search_type}:${this.mrp.search_query}` : ''}&sort_by=nama:${this.mrp.sort_by}`,
                        {
                            'headers' : {
                                'Accept': 'application/vnd.api+json',
                                'Authorization': `Bearer ${token}`
      ...
      return sanitizeUrl(dirtyUrl);
        ...
        async uploadMrp(index) {
            ...
            formData.append("tipe_objek_id", 2);
        ...
        async getVersiDokumen() {
            try {
                const token = window.localStorage.getItem('access_token')
                let versi_dokumen = await this.$api.get(`${this.$store.state.simulation_base_url}/versidokumen?jenis_dokumen=1&sub_jenis_dokumen=3&sort_by=id:desc&offset=10000`,
                        {
                            'headers' : {
                                'Accept': 'application/vnd.api+json',
                                'Authorization': `Bearer ${token}`
                ...
                if (versi_dokumen.data.data) {
                    ...
                    this.list_versi_dokumen_upload_filtered = 
                        this.list_versi_dokumen_upload
                            .filter(x => x.nama.includes('Motor'));
                }
            } catch (error) {
                throw new Error("Something went wrong " + error);
            }
        ...
        async addVersion (newVersion) {
            let data = {
                data: {
                    type: 'string',
                    attributes: {
                        nama: 'Motor - ' + newVersion,
                        jenis_dokumen_id: 1,
                        sub_jenis_dokumen_id: 3,
                    }
                }
            }
            try {
