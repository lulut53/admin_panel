<template>
  <div>
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-6 col-7">
          <h6 class="h2 d-inline-block mb-0">Tag Produk</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              ...
              <li class="breadcrumb-item" aria-current="page">Tag Produk</li>
            </ol>
          </nav>
        </div>
              ...
              <div>
                ...
                <el-table :data="item"
                  ...
                  <el-table-column label="Nama Kategori"
                    ...
                    <template v-slot="{row}">
                      <!-- {{row.attributes.nama}} -->
                      <!-- <router-link :to="{ name: 'detail tag produk', params: { id: row.id }}"> -->
                        {{row.attributes.nama}}
                        <!-- </router-link> -->
                    </template>
                  </el-table-column>
    ...
    <form @submit.prevent="simpanTags">
      <el-dialog :visible.sync="showModalAdd" title="Kelola Data Tag Kategori">
        <!-- <transition name="fade" mode="out-in"> -->
          <div key="1" v-if="!isModalProcess">
            <div class="row mb-4">
              <label for="nama" class="col-md-4 col-form-label form-control-label">Nama Kategori</label>
              <div class="col-md-8">
                <label class="col-md-4 col-form-label form-control-label">{{ tmpAddData.namaProduk }}</label>
              </div>
            </div>
            <div class="row mb-4">
              ...
              <div class="col-8">
                <div>
                  <multiselect v-model="tmpAddData.tags" tag-placeholder="Add this as new tag" placeholder="Add a tag" label="name" track-by="code" :multiple="true" :options="optionstag" :taggable="true" @tag="addTag">
                    <span slot="noOptions">Ketikkan tag baru, tekan enter untuk menambah</span>
                  </multiselect>
                </div>
              </div>
            </div>
        ...
        <template slot="footer" v-if="!isModalProcess">
          ...
          <el-button type="success" @click="simpanTags">Tambah</el-button>
        </template>
      </el-dialog>
    </form>
  </div>
...
export default {
  ...
  computed: {
    ...mapGetters('produk', ['produks', 'tag_produks', 'tag_by_produk', 'included_produk']),
  },
  methods: {
    ...mapActions('produk', [
      'getProduks',
    ...
    async getData() {
      ...
      let res = JSON.parse(JSON.stringify(await this.getProduks()));
      if (res.name != 'Error') {
        this.item = JSON.parse(JSON.stringify(this.produks))
    ...
    async editModal(row) {
      ...
      await this.getTagByProduk(row.id)
    ...
    async simpanTags() {
      ...
      const payload = {
        id: this.tmpAddData.id,
        data: this.tmpAddData.tags
      }

      try {
        await Promise.all([
          this.updateTagProduk(payload),
        ])
      } catch (error) {
        this.showModalAdd = false
        this.isModalProcess = false

        swal("Gagal", "Gagal mengganti point", "error")
        .then(function () {
          // that.getData()
        })
      } finally {
        ...
        swal("Sukses", "Berhasil kelola tags", "success")
        .then(function () {
          that.getData()
        })
      }
