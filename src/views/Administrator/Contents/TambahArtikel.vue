    ...
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-8 col-8">
          <h6 class="h2 d-inline-block mb-0">Tambah Artikel Baru</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              ...
              <li class="breadcrumb-item" aria-current="page">Tambah Artikel Baru</li>
            </ol>
          </nav>
        </div>
...
export default {
  ...
  watch: {
    'tmpAddData.nama': function (val) {
      this.tmpAddData.slug = val.split(' ').join('-')
    }
  },
  computed: {
  ...
  methods: {
    ...
    async getData() {
      ...
      let res = await this.getKategoris()
      if (res.name != 'Error') {
    ...
    async submit() {
      ...
      let res = await this.createBerita(this.tmpAddData)

      this.showModalProcess = false

      if (res.name != 'Error') {
        const that = this
        swal( 'Berhasil', 'Berhasil menambahkan artikel baru', 'success' )
        .then((result) => {
          this.$router.push('/administrator/contents/news')
        });
      } else {
        swal( 'Gagal', res.response.status == 413 ? "Gagal menambahkan artikel baru, ukuran file teralu besar. (maks 5MB)" : 'Gagal menambahkan artikel baru', 'error' )
        .then((result) => {
          // that.getData()
        });
      }
