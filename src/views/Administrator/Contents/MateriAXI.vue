<template>
  <div>
    <base-header class="pb-6" type="">
      <div class="row align-items-center py-4">
        <div class="col-lg-6 col-7">
          <h6 class="h2 d-inline-block mb-0">Materi AXI</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links">
              ...
              <li class="breadcrumb-item" aria-current="page">Materi AXI</li>
            </ol>
          </nav>
        </div>
            ...
            <div key="2" v-if="isShow">
              <div>
                ...
                <el-table :data="item"
                  ...
                  <el-table-column label="Ukuran File"
                                min-width="150px"
                                prop="size"
                                sortable="custom">
                    <template v-slot="{row}">
                        {{row.attributes.size | convertSize}} MB
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="id" min-width="250px" align="right" label="Aksi">
                    <div slot-scope="{$index, row}" class="d-flex">
                      <el-tooltip content="Download" placement="top">
                        <base-button
                          @click.native="downloadMateri(row.attributes.path)"
                          class="like btn-link text-white"
                          type="info"
                          size="sm"
                          icon>
                          <i class="text-white ni ni-cloud-download-95"></i> Download
                        </base-button>
                      </el-tooltip>
                      ...
                      <el-tooltip content="Delete" placement="top">
                        <base-button
                          @click.native="hapusMateri(row)"
                          class="like btn-link text-white"
                          type="danger"
                          size="sm"
                          icon>
                          <i class="text-white fa fa-edit"></i> Hapus
                        </base-button>
                      </el-tooltip>
                    </div>
                  </el-table-column>
    ...
    <el-form @submit.prevent="editData">
      <el-dialog :visible.sync="showModalEdit" title="Edit Materi AXI">
        <!-- <transition name="fade" mode="out-in"> -->
          <div key="1" v-if="!isModalProcess">
            <div class="form-group">
              <div class="row mb-4">
                <label for="nama" class="col-md-4 col-form-label form-control-label">Nama Materi</label>
                <div class="col-md-8">
                  <el-input v-model="currentRow.nama" />
                </div>
              </div>
              <div class="row">
                <label for="file" class="col-md-4 col-form-label form-control-label">Upload File</label>
                <div class="col-md-8 pb-4">
                  <file-input id="file" @change="fileChange" class="form-group" required></file-input>
                </div>
              </div>
            </div>
          </div>
    ...
    <el-form @submit.prevent="addData">
      <el-dialog :visible.sync="showModalAdd" title="Tambah Materi AXI">
        <!-- <transition name="fade" mode="out-in"> -->
          <div key="1" v-if="!isModalProcess">
            <div class="row mb-4">
              <label for="nama" class="col-md-4 col-form-label form-control-label">Nama Materi</label>
            ...
            <div class="row">
              <label for="file" class="col-md-4 col-form-label form-control-label">Upload File</label>
              <div class="col-md-8 pb-4">
                <file-input id="file" @change="fileChange" class="form-group" required accept=".pdf"></file-input>
              </div>
            </div>
          </div>
          <div key="2" v-if="isModalProcess" class="text-center p-5" style="display: flex; margin-top: 20px !important; justify-content: center;">
...
export default {
  ...
  computed: {
    ...mapGetters('materiAxi', ['materiAxis','included']),
  },
  methods: {
    ...mapActions('materiAxi', [
      'getDownloadMateri',
      'getMateriAxis',
      'getMateriAxisQueried',
      'createMateriAxi',
      'updateMateriAxi',
      'hapusMateriAxi'
    ]),
    ...
    async getData() {
      ...
      let res = JSON.parse(JSON.stringify(await this.getMateriAxisQueried(data)));
      if (res.name != 'Error') {
        this.item = JSON.parse(JSON.stringify(this.materiAxis))
    ...
    async assignData() {
      ...
      let res = await this.getMateriAxisQueried(data)
      if (res.name != 'Error') {
        this.item = JSON.parse(JSON.stringify(this.materiAxis))
        this.dataIncluded = this.included ? JSON.parse(JSON.stringify(this.included)) : this.included
    ...
    async editData() {
      ...
      this.currentRow.file = this.fileMateri ? this.fileMateri : null
      
      const res = JSON.parse(JSON.stringify(await this.updateMateriAxi(this.currentRow)))
      ...
      if (res.name != 'Error') {
        ...
        swal("Error", res.response.status == 413 ? "Edit data gagal, ukuran file teralu besar. (maks 50MB)" : "Edit data gagal, mohon coba lagi", "error")
        .then(function () {          
        }).catch(function (error){
        })
      }
    ...
    async addData() {
      ...
      let payload = {
        nama: this.nama,
        file: this.fileMateri
      }
      const res = await this.createMateriAxi(payload);
      this.fileMateri = '';
      ...
      if (res.name != 'Error') {
        swal("Sukses", "Berhasil membuat data materi baru", "success")
        .then(function () {
          location.reload();
        }).catch(function (error){
        })
      } else {
        swal("Error", res.response.status == 413 ? "Tambah data gagal, ukuran file teralu besar. (maks 50MB)" : "Tambah data gagal, mohon coba lagi", "error")
        .then(function () {
        }).catch(function (error ){
        })
      }
    ...
    editModal(row) {
      ...
      this.currentRow.nama = row.attributes.nama
    },
    ...
    async hapusMateri(id) {
      ...
      const res = JSON.stringify(await this.hapusMateriAxi(id))
      this.isModalProcess = false
      const that = this
