              ...
              <div key="2" v-if="isShow">
                <div>
                  ...
                  >
                    ...
                    <el-table-column label="Nama AXI" min-width="200px" prop="namaAxi">
                      <!-- <template v-slot="{row}"> -->
                      <!-- <router-link
                          :to="{ name: 'details pendaftaran axi', params: { id: row.id }}"
                      >{{ getIncluded('profiles', row.relationships.profile.data, 'nama') }}</router-link>-->
                      <!-- <a v-bind:href="'/administrator/requests/'+ row.id +'/detail'">
                            {{row.name}}
                      </a>-->
                      <!-- </template> -->
                    </el-table-column>
                    <el-table-column label="Jenis AXI" min-width="200px" prop="jenisAxi">
                    </el-table-column>
                    <el-table-column label="AXI Mentor" min-width="150px" prop="axiIdReff">
                      <!-- <template
                        v-slot="{row}"
                      >{{ row.attributes.axi_id_reff ? row.attributes.axi_id_reff : '-' }}</template>-->
                    </el-table-column>
                    <el-table-column label="Cabang Daftar" min-width="200px" prop="cabangTerdaftar">
                      <!-- <template
                        v-slot="{row}"
                    ...
                    <el-table-column label="Tanggal Daftar" min-width="180px" prop="tanggalDaftar">
                      <template v-slot="{row}">{{ row.tanggalDaftar | convertDate }}</template>
                    </el-table-column>
                    <el-table-column label="Dibuat Pada" min-width="180px" prop="dibuatPada">
                      <template v-slot="{row}">{{row.dibuatPada | convertDate}}</template>
                    </el-table-column>
                    <el-table-column label="Status" min-width="150px" prop="status_registrasi">
                      <template v-slot="{row}">
                        <badge type="default" v-if="row.status_registrasi == 'BARU'">{{row.status_registrasi}}</badge>
                        <badge type="primary" v-else>{{row.status_registrasi}}</badge>
                      </template>
                    </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      filterOptions: [
        ...
        { value: "AXI PERORANGAN", name: "AXI PERORANGAN" },
        { value: "AXI PEDAGANG", name: "AXI PEDAGANG" },
        { value: "AXI REFO", name: "AXI REFO" },
        { value: "AXI KBA", name: "AXI KBA" },
        { value: "AXI CBD", name: "AXI CBD" },
      ],
      filterByJenisAxi: "Semua",
      columnSearched: [
        // { name: 'All', value: 'all' },
        { name: "Nama AXI", value: "nama" },
        { name: "AXI Mentor", value: "mentor" },
  ...
  computed: {
    ...mapGetters("axi", ["axis", "included", "cabang", "listRegAxi"])
  },
  methods: {
    ...mapActions("axi", [
      "getAxis",
    ...
    async getData() {
      ...
      data.searchAll = this.searchData;
      data.pageNumber = 1;
      data.filterByJenisAxi = this.filterByJenisAxi;

      let res = JSON.parse(JSON.stringify(await this.getAxiRegList(data)));
      // console.log(res.data);

      if (res.data != "") {
        this.item = JSON.parse(JSON.stringify(this.listRegAxi));
    ...
    async assignData() {
      ...
      let res = await this.getAxiRegList(data);
      if (res.data != "") {
        this.item = JSON.parse(JSON.stringify(this.listRegAxi));
  ...
  watch: {
    ...
    filterByJenisAxi:function() {
      this.searchByJenisAxiCanged();
    },
